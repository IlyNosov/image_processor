...
template <typename T>
void reading_and_writing::Writer::NumToBytes(T number, unsigned char* bytes) {
    *bytes = number;
...
void reading_and_writing::Writer::WriteHeader(unsigned char* file_header, size_t file_size) {
...
void reading_and_writing::Writer::WriteInfoHeader(unsigned char* info_header, size_t width, size_t height) {
...
void reading_and_writing::Writer::Write(const Image& img) {
    ...
    for (size_t y = 0; y < img.GetHeight(); ++y) {
        ...
        out_img.write(reinterpret_cast<char*>(empty_pix), static_cast<std::streamsize>(GetPaddedSize(img.GetWidth())));
    }
    out_img.close();
}
}  // namespace reading_and_writing
